...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
    ...
    //重新加载登录信息
    LAYIM.prototype.reloadInfo = function(content){ 
      return  init(baseOption);
    };

    //重新加载登录信息
    LAYIM.prototype.loginOut = function(content){ 
    ...
    /** 获取web代理根路径 */
    function serverPath (){
      return localStorage.getItem("layim-web");
    }
    ...
    var elemTpl = ['<div class="layui-layim-main">'
      ,'<div class="layui-layim-info">'
        ...
        ,'<div class="layui-layim-status">'
          ...
          ,'{{# } else if(d.mine.status === "leave") { }}'
          ,'<span class="layui-icon layim-status-leave" layim-event="status" lay-type="show">&#xe60f;</span>'
          ,'{{# } }}'
          ,'<ul class="layui-anim layim-menu-box">'
            ...
            ,'<li {{d.mine.status === "leave" ? "class=layim-this" : ""}} layim-event="status" lay-type="leave"><i class="layui-icon">&#xe605;</i><cite class="layui-icon layim-status-leave">&#xe60f;</cite>离开</li>'
          ,'</ul>'
        ,'</div>'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
      ,'<div class="layui-unselect layim-chat-title">'
        ,'<div class="layim-chat-other">'
          ,'<img class="layim-{{ d.data.type }}{{ d.data.id }}" src="'+serverPath()+'{{ d.data.avatar }}"><span class="layim-chat-username" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }} {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
    ...
    var elemAddTpl = ['<div class="layim-add-box">'
      ,'<div class="layim-add-img"><img class="layui-circle" src="'+serverPath()+'{{ d.data.avatar }}"><p>{{ d.data.name||"" }}</p></div>'
    ...
    var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
      ,'<div class="layim-chat-user"><img src="'+serverPath()+'{{ d.avatar }}"><cite>'
      ,'{{# if(d.mine){ }}'
    ...
    var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="'+serverPath()+'{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
    ...
    var post = function(options, callback, tips){
      options = options || {};
      //TODO 避开 token，loginid 被重置后获取的旧的值导致登录取数据失效
      let token = localStorage.getItem("layim-token");
      let loginid = localStorage.getItem("userId");
      let urls=serverPath()+options.url;
      return $.ajax({
        url: urls
        ,type: options.type || 'get'
        ,data: $.extend({},{apptoken:loginid+"-"+token},options.data)
        ,dataType: options.dataType || 'json'
        ,cache: false
        ,success: function(res){
          res.code == 0   ? callback && callback(res.data||{}) : layer.msg(res.msg || ((tips||'Error') + ': LAYIM_NOT_GET_DATA'), { time: 5000  });
        },error: function(err, msg){
          window.console && console.log && console.error('LAYIM_DATE_ERROR：' + msg+urls);
          //callback({});
        }
      });
    };
    ...
    var layimMain, popim = function(content){
      //console.log('窗口变化回调',content);
      layui.layim.loginOut();//销毁页面残留上次登录信息；
      return layer.open({
        type: 1
        ,area: ['260px', '520px']
    ...
    var layimClose, popmin = function(content){
      ...
      return layer.open({
        ...
        ,offset: 'rb'
        ,resize: false
        ,content: '<img src="'+serverPath()+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'"><span>'+ (content||cache.base.title||'我的LayIM') +'</span>'
    ...
    var setChatMin = function(newMsg){
      ...
      setChatMin.index = layer.open({
        ...
        ,resize: false
        ,area: ['182px', '50px']
        ,content: '<img id="layui-layim-min" src="'+serverPath()+ thatChat.avatar +'"><span>'+ thatChat.name +'</span>'
        ,success: function(layero, index){
    ...
    var addList = function(data){
      ...
      if(data.type === 'friend'){
        ...
        if(cache.friend[data.groupIndex]){
    ...
    var anim = 'layui-anim-upbit', events = {
      ...
      status: function(othis, e){
        ...
        if(type === 'show'){
          ...
          prev.removeClass('layim-status-online');
          prev.removeClass('layim-status-hide'); 
          prev.removeClass('layim-status-leave'); 
          
          prev.addClass('layim-status-'+type);
          ...
          if(val === ''){
            ...
            if(data.length > 0){
              for(var l = 0; l < data.length; l++){
                html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+serverPath()+ data[l].avatar +'"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
              }
            } else {
      ...
      ,msgbox: function(){
        ...
        return events.msgbox.index = layer.open({
          ...
          ,resize: false
          ,content: serverPath()+cache.base.msgbox+"?apptoken="+localStorage.getItem("userId")+"-"+localStorage.getItem("layim-token") 
        });
      }
      //弹出查找页面
      ,find: function(){
            ...
            ,success: function(layero){
              ...
              post(members, function(res){
                layui.each(res.list, function(index, item){
                  li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="'+serverPath()+ item.avatar +'"><cite>'+ item.username +'</cite></a></li>';
        ...
        layui.upload.render({
          ...
          ,done: function(res){ 
            if(res.code == 0){
              ...
              sendMessage();
            } else if(res.success=='1'){//njmind
              let urlp=upload.url.replace("upload","down/");
              if(type === 'images'){
                focusInsert(thatChat.textarea[0], 'img['+ (urlp+res.v_attach_id||'') +']');
              } else if(type === 'file'){
                focusInsert(thatChat.textarea[0], 'file('+ (urlp+res.v_attach_id||'') +')['+ (res.v_orig_name||'下载文件') +']');
              }
              sendMessage();
            } else {
      ...
      ,chatLog: function(othis){
        ...
        return events.chatLog.index = layer.open({
          ...
          ,content: serverPath()+ cache.base.chatLog +"?apptoken="+localStorage.getItem("userId")+"-"+localStorage.getItem("layim-token")
                                       +'&id=' + thatChat.data.id + '&type=' + thatChat.data.type 
        });
      }
