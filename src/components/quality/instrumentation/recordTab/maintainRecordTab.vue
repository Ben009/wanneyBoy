<template>
    <div>
        <div v-if="!detailInfo">
            ...
            <div>
                ...
                <el-table :data="tableData"
                    ...
                    <el-table-column key="maintainWay" label="维护方式" prop="maintainWay" :formatter="maintainWayFormat" v-if="headObj.maintainWay"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
                    <el-table-column key="validateWay" label="验证方式" prop="validateWay" :formatter="(row,column,value)=>{return value == '1'?'校准/检定':'自校准/验证'}" v-if="headObj.validateWay"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
                    <el-table-column key="userName" label="经手人" prop="userName" v-if="headObj.userName"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
                    <el-table-column key="maintainDate" label="保养日期" prop="maintainDate" :formatter="dateFormat" v-if="headObj.maintainDate"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
                    <el-table-column key="maintainCycle" label="保养周期(月)" prop="maintainCycle"  v-if="headObj.maintainCycle"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
                    <el-table-column key="estimatedTime" label="预计下次保养日期" prop="estimatedTime" :formatter="dateFormat" v-if="headObj.estimatedTime"  show-overflow-tooltip   :render-header="QJrenderHeader" > </el-table-column>
        ...
        <instrumentation-tab v-if="detailInfo" type="maintainRecordTab"  :standardId="detailInfo" @goBack="back"></instrumentation-tab>
    </div>
</template>
<script>
import Vue from "vue";
...
import param from "./maintainRecordTabParam.js";
...
export default {
  ...
  methods: {
    ...
    getTableList(){
      ajaxRequest('get', 'back/instrumentation/retrieveMaintain', {
        page: this.page,
        rows: this.rows,
    ...
    maintainWayFormat(row,column,value){
