        ...
        <el-col :span="10">
          <el-form-item label="类型" prop="type">
            <el-select
              ...
              @change="change"
              style="width:100%;" >
              <el-option label="计量标准" :value="0"></el-option>
              <el-option label="项目维护" :value="2"></el-option>
              <el-option label="CNAS管理" :value="1"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <div style="margin-top:10px;">
          <el-table :max-height="QJTableMaxHeight"
            ...
            <el-table-column prop="projectName" label="项目名称" :render-header="renderHeader"></el-table-column>
            <el-table-column prop="projectType" label="项目类别" :render-header="renderHeader"> </el-table-column>
            <el-table-column prop="remark" label="备注" :render-header="renderHeader"></el-table-column>
          </el-table>
          ...
          <el-table :max-height="QJTableMaxHeight"
            ...
            <el-table-column prop="standardCerNo" label="计量标准证书号" :render-header="renderHeader"></el-table-column>
            <el-table-column prop="measuringRange" label="测量范围" :render-header="renderHeader"></el-table-column>
          </el-table>
          ...
          <el-table :max-height="QJTableMaxHeight"
            ...
            @row-click="checkThisQui">
            <el-table-column type="index" fixed label="序号" width="80" align="center"></el-table-column>
            <el-table-column prop="name" label="项目名称" :render-header="renderHeader"></el-table-column>
            <el-table-column prop="nameE" label="项目名称(英文)" :render-header="renderHeader"></el-table-column>
            <el-table-column prop="typeName" label="项目类别"  :render-header="renderHeader"></el-table-column>
          </el-table>
        </div>
...
<script>
  ...
  import paramJson from './selectProjectParam';
  //引入jquery
  import $ from 'jquery';
  import ElRow from "@/common-el/packages/row/src/row";
  import MindPagination from "../../../common/packages/pagination/src/pagination";
...
//        },
      ...
      clearCheck(){
        this.tempIds = this.recheckTecBas(this.index1)
      },
      ...
      getTableList() {
        ...
        var url;
        if(this.value=='2'){
          url='back/personQufication/selectProject';
        }else if(this.value=='0'){
          url='back/mStandard/retrieve';
        } else if(this.value=='1'){
          url='back/cnas/retrieve';
        }
...
//            orderProperty:'t.id',
      ...
      checkThisQui(row){
        this.checkRow(row,this.index1,this.index,this.value)
        this.callback()
      },
      search(){
        ...
        if(this.multipleSelection.length>0){
          if(this.value=='0'){
            for(var i=0;i<this.multipleSelection.length;i++){
              let obj={};
              obj.projectName=this.multipleSelection[i].standardName;
              obj.linkId=this.multipleSelection[i].id;
              obj.source=this.value;
              list.push(obj);
            }
          }
          if(this.value=='1'){
            for(var i=0;i<this.multipleSelection.length;i++){
              let obj={};
              obj.projectName=this.multipleSelection[i].name;
              obj.linkId=this.multipleSelection[i].id;
              obj.source=this.value;
              list.push(obj);
            }
          ...
          if(this.value=='2'){
            for(var i=0;i<this.multipleSelection.length;i++){
              let obj={};
              obj.projectName=this.multipleSelection[i].projectName;
              obj.linkId=this.multipleSelection[i].id;
              obj.source=this.value;
              list.push(obj);
            }
          ...
          console.log(list)
          this.checkRows(list,this.index1,this.index,this.value)
          this.callback()
        }
