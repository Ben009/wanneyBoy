<template>
  <div :style="{display: 'flex',height:innerHeight+'px'}" class="firstAuditCss" ref="firstAudit-page">
      ...
      <div class="certificateContent" :style="{'width':calcWidth+'px','padding':'0px 5px','z-index':'1'}">
        <CertificateTemplateFirstAuditDetail v-if="isRouterAlive" :mlId="mlId" :mbId="mbId"
...
<script>
  ...
  import CertificateTemplateFirstAuditDetail from './certificateTemplateFirstAuditDetail';
...
/*   import {Container,Aside,Main} from 'element--ui';
  ...
  export default {
    name: "CertificateTempMainFirstAudit",
    components:{
      /** 模块对象 import 进来 后的别名  */
      CertificateListTree,CertificateTemplateFirstAuditDetail,
    },
    data(){
      //rentun中的变量仅针对当前组件有效
      return{
    ...
    // 查询当前文件夹一级未审核的数据列表
     getUnauditedList(mlId){
       if(mlId){
         var url = "" ;
//         if(mlId == 0 ){
//           url = "back/certificateTemp/getMbByMbName" ;
//         }else {
           url = "back/certificateTemp/getCertificateTempListByMlId" ;
//         }
