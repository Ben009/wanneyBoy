...
//        this.showZsDetail();
      ...
      printCertificate(data,backPrint,isBatch){
        ...
        var fileId = "";
        var printCopies = 1 ;
        var actionContent = "证书打印（分散打印）";
        if(!backPrint && !isBatch){
          actionContent = "证书打印（分散打印）";
        }else if(backPrint && !isBatch){
          actionContent = "证书备份打印（分散打印）";
        }else if(!backPrint && isBatch){
          actionContent = "证书批量打印（分散打印）";
        }else if(backPrint && isBatch){
          actionContent = "证书备份批量打印（分散打印）";
        }
