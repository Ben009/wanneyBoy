...
//  Vue.use(Col);
  ...
  export default {
    name: "CertificateAuditJdyDetailMain",
    components:{
      /** 模块对象 import 进来 后的别名  */
      ...
      certificateSH(){
        ...
        for(var i=0;i<this.auditStatusTemp.length;i++){
          if(this.auditStatusTemp[i].auditRyId == window.localStorage.userId && this.auditStatusTemp[i].auditRyStatus != 1 && flag){
            flag = this.$refs.certificateAddDetail.$refs.certificateXxy.checkSignnaturePasswordBack(true,"0","0",i,this.auditStatusTemp[i].auditRyId ,this.certificateId,"")
          }
        }
        if(flag){
...
//        var auditObj = this.$refs.certificateAddDetail.$refs.certificateXxy.certificateAudit("0", "0", "", "", this.certificateId, "审核成功");
...
//        });
      ...
      certificateSHBatch(){
        ...
        var auditObj = this.$refs.certificateAddDetail.$refs.certificateXxy.certificateAuditBatch(this.checkAuditDjhList,'0',"审核成功");
        if(auditObj != false){
