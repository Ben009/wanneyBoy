<template>
  <div class="certArea" ref="certArea-page" :style="{height:innerHeight+'px'}">
        ...
        <div class="topButton">
          <el-form :inline="true">
            <el-col :span="24">
              ...
              <el-col :span="6" style="text-align: center;margin-top:7px;">
                <template>
                <span>
                    {{certificateTotal}}
                  </span>
                  <el-tooltip content="当前需要修改的证书数量" placement="top">
          ...
          <el-table :data="certificateData" :show-header="false" ref="certificateTable" @row-click='certificateDataClick' :max-height="ZSTableMaxHeight" >
            <el-table-column align='left'>
              <template slot-scope="scope" align="left">
                <!--<el-checkbox  :label="scope.row.registrationNo"  :key="scope.row.registrationNo" @change="certificateDataClick(scope.row.registrationNo,scope.row.certificateNumber)" >-->
      ...
      <div class="certRightArea" :style="{'width':calcWidth+'px','padding':'0px 5px','z-index':'1'}">
        <CertificateEidtBeforePzDetailMain
                                  ref="certificateEidtBeforePzDetailMain"
...
//  } from "element-ui";
  ...
  import CertificateEidtBeforePzDetailMain from './certificateEidtBeforePzDetailMain.vue';
...
//        searchDialogShow: false,
    ...
    components: {
      ElCol,CertificatesList,CertificateEidtBeforePzDetailMain,

    },
    computed:{
      ZSTableMaxHeight(){
        return  this.$store.state.admin.window_innerHeight-113;
      },
    },
    watch:{
      innerWidth(v){
    ...
    methods: {
      ...
      search() {
        ...
        this.getHyySjdList(paramters)
      },
      getHyySjdList(paramters) {
        paramters = paramters == null ? {page: this.page, rows: this.rows, total: 0} : paramters;
        console.log(paramters,'paramters')
        ajaxRequest("get", "back/certificate/getCertificateSjdQueryModifyList", paramters, res => {
          if (res.code === 200) {
            ...
            if(this.registrationNoTemp != null ){
              this.certificateDataClick({registrationNo:this.registrationNoTemp});
            }else {
      ...
      getCertificateList(registrationNo) {
        ajaxSyncRequest("get", "back/certificate/getCertificateModifyList", {registrationNo:registrationNo,type:this.queryType}, res => {
          if (res.code === 200) {
...
//            if(this.$refs.certificateEidtBeforePzDetailMain) {
//              var showId = this.$refs.certificateEidtBeforePzDetailMain.$refs.certificateAddDetail.getCertificateId();
