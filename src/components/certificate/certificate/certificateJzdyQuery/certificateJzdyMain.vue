  ...
  <div>
      <div>
        <el-table :data="certificateList" style="width: 100%">
          ...
          <el-table-column prop="userName" label="添加人" show-overflow-tooltip >
          </el-table-column>
...
//        this.showZsDetail();
      ...
      printCertificate(data,backPrint,isBatch){
        ...
        var fileId = "";
        var printCopies = 1 ;
        var actionContent = "证书打印（集中打印）";
        if(!backPrint && !isBatch){
          actionContent = "证书打印（集中打印）";
        }else if(backPrint && !isBatch){
          actionContent = "证书备份打印（集中打印）";
        }else if(!backPrint && isBatch){
          actionContent = "证书批量打印（集中打印）";
        }else if(backPrint && isBatch){
          actionContent = "证书备份批量打印（集中打印）";
        }
