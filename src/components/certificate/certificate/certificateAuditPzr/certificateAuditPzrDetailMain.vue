<template>
  <div ref="certificate-page">
        <div class="topButton cerCommonStyle" ref="topBut">
          ...
          <el-button :disabled="isSaved" type="primary" @click="pzrButtonClick(false)" size="small"  class="certificateButton">审核</el-button>
          <el-button type="primary" size="small" v-if="certificateApprovalTwoShow ==1" @click="pzrButtonClick(true)"  class="certificateButton">备用批准</el-button>
          <el-button type="primary" @click="certificateSHBatch" v-if="canSHBatch" size="small"  class="certificateButton">批量审核</el-button>
          <el-button type="primary" @click="certificateTHShow" size="small"  class="certificateButton">退回</el-button>
    ...
    <CommonInputDialog
      ...
      label="退回原因"
      placeholder="请输入退回原因"
      :callback="certificateTHCallback"
      v-if="certificateSHShowFlag"
    >
...
//  } from 'element-ui';
  ...
  import CommonInputDialog from '../../common/commonInputDialog.vue';
...
//  Vue.use(Col);
  ...
  export default {
    ...
    components:{
      /** 模块对象 import 进来 后的别名  */
      CertificateAddDetail,CommonInputDialog,
    },
    props: {
      //证书ID
...
//      },
      ...
      certificateTHShow(){
        this.certificateSHShowFlag = true ;
      },
      certificateTHCallback(mes){
        this.certificateSHShowFlag = false ;
        if(!mes){
          return ;
        }
        this.certificateTH(mes);
      },
      certificateSH(){
        var auditObj = this.$refs.certificateAddDetail.$refs.certificateXxy.certificateAudit("0", "2", "", window.localStorage.userId, this.certificateId, "审核成功");
...
//        });
      ...
      certificateTH(mes){
        var auditObj = this.$refs.certificateAddDetail.$refs.certificateXxy.returnCertificatePzr(this.certificateId,mes, "退回成功");
//        ajaxSyncRequest("get", "back/certificate/getCertificateListBySjd", {registrationNo:this.registrationNo}, res => {
          if(auditObj != false){
            this.shCallBack("one");
            //父页面刷新证书列表
...
//        });
      ...
      certificateSHBatch(){
        ...
        var auditObj = this.$refs.certificateAddDetail.$refs.certificateXxy.certificateAuditBatch(this.checkAuditDjhList,'2',"审核成功");
        if(auditObj != false){
