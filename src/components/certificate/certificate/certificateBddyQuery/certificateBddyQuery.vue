...
<script>
  ...
  import CertificateBddyMain from './certificateBddyMain.vue';
...
//        certificateTotal:0,
    ...
    components: {
      ElCol,CertificatesList,CertificateBddyMain,
...
//      },
      getPrintSjdList(paramters) {
        ...
        ajaxSyncRequest("get", "back/certificatePrintQuery/getCertificateBdPrintSjdList", paramters, res => {
          if (res.code === 200) {
...
//            this.certificateTotal = res.map.certificateTotal;
      ...
      getCertificateList(registrationNo) {
        var flag = false;
        ajaxSyncRequest("get", "back/certificatePrintQuery/getCertificateBdPrintList", {registrationNo:registrationNo}, res => {
          if (res.code === 200) {
...
//      },
      ...
      batchPrint(backPrint){
        ...
        this.$refs.certificateBddyMain.continuePrint = true ;
...
//            for (var i=0;i<this.certificateList.length ;i++) {
            console.log(this.registrationNo + "打印第"+this.batchPrintZsIndex+"分证书" + this.certificateList[this.batchPrintZsIndex].zsbh);
              this.$refs.certificateBddyMain.printCertificate(this.certificateList[this.batchPrintZsIndex], backPrint, true);
...
//            this.search();
      ...
      stopPrint(){
        this.$refs.certificateBddyMain.stopPrint();
      },
    },
    filters: {
