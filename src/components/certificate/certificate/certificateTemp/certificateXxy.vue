<template>
  <div>
    <div>
      <el-col :span="24">
        <!--<el-button type="primary" @click="relationXxy" size="small">关联信息页主模</el-button>-->
        <!--<el-select v-model="certificateJdzqTemp" style="width:100px"-->
        ...
        <!--&gt;-->
          ...
          <!--</el-option>-->
        <!--</el-select>-->
        <div ref='xxyContent' id="xxyContent" class="cerEditorStyle">

        </div>
      </el-col>
    </div>
    <CertificateTreeDialog :queryType="1" :xxyZmbId="0" v-if="!!treeDialogShow" :callback="treeDialogClose" ></CertificateTreeDialog>
...
//    },
  ...
  data() {
    return {
      ...
      certificateJdzqArr : [] ,
      //props中的值不能修改，创建临时对象
//      certificateJdzqTemp:12 ,
      certificateJdzq:12 ,
      xxyTotalPage : 0 ,
//      canChangeCertificateGcInfo:false,//按纽_证书编辑时可更换技术规程、计量标准及标准器
//      canModifyCertificateGcInfo:false,//按钮_证书编辑时可右键修改技术规程、计量标准及标准器
      checkSignnaturePasswordShow:false,
    };
  },
  watch: {
//    certificateJdzq(){
//      this.certificateJdzqTemp = this.certificateJdzq ;
//    },
  },
  created() {
    //获得检定周期
  ...
  mounted() {
//    this.certificateJdzqTemp = (this.certificateJdzq)?this.certificateJdzq:this.certificateJdzqTemp ;
  },
  computed:{
  ...
  methods: {
    ...
    treeDialogClose(xxyZmbId) {
      //隐藏弹出层
      this.treeDialogShow = false;
      if (xxyZmbId) {
        this.getXxyCont(xxyZmbId);
      }
    },
    leftClick() {
      //绑定左击事件
      ...
      $("#xxyContent .zs_click").click(function(event) {
        if(_this.certificateCanEdit) {
          ...
          if($(that).attr("name").startsWith("zs_jsgc_") || $(that).attr("name").startsWith("zs_jlbz_") || $(that).attr("name").startsWith("zs_jlbzq_")) {
            if(_this.canChangeCertificateGcInfo ) {
              //技术规程
...
//          //无权限可修改
//          if(!_this.canModifyCertificateGcInfo ){
//            if($(this).attr("name").indexOf("zs_jsgc") == 0 ||
//              $(this).attr("name").indexOf("zs_jlbz") == 0 ||
//              $(this).attr("name").indexOf("zs_jlbzq") == 0
//            ){
//              return;
//            }
//          }
    ...
    getXxyCont(xxyZmbId) {
      console.log(xxyZmbId,'xxyZmbId')
      //重置页面
      this.initXxyCont();

      this.xxyZmbId = xxyZmbId;
      if (!xxyZmbId) {
        return;
      }
      this.$emit('getXxyCont', xxyZmbId );
    },
//    clearContent(obj) {
//      $("#xxyContent").html("");
//    },
    setContent(xxyContent) {
      console.log(document,document.getElementById("#xxyContent"),$("#xxyContent"),'1111111111111111111111------------------------------$("#xxyContent")')
...
//        ajaxSyncRequest('get', 'back/depart/retrieve/', {isDelete: 0}, res => {
//          if (res.code == 200) {
            var _this = this;
//            this.jcbmList = res.rows;
//            //追加下拉选项
//            for (var i = 0; i < this.jcbmList.length; i++) {
//              $("#xxyContent [name='zs_dept_select']").append("<option value='" + this.jcbmList[i].id + "'>" + this.jcbmList[i].departName + "</option>");
//            }
...
//        })
    ...
    getPzrList(depId){
      ...
      ajaxSyncRequest('get','back/certificateCommon/getDepartPzrList/'+depId , null ,res=>{
        if (res.code==200) {

          //删除除第一个选项卡的下拉框
          var options = $("#xxyContent [name='zs_pzr_select']").eq(this.indexEle).children('option');
          for(var i=1 ;i<options.length;i++ ){
            options[i].remove();
          }
          //追加人员列表
          this.dPzRList=res.rows;
          for(var i=0;i<this.dPzRList.length;i++){
            $("#xxyContent [name='zs_pzr_select']").eq(this.indexEle).append("<option value='"+this.dPzRList[i].userId+"'>"+this.dPzRList[i].userName+"</option>");
          }
        }
      })
    },
        ...
        if($(obj).val() != $("#xxyContent [name='zs_jdy_id']").eq(this.indexEle).val()){
          ...
          if($("#xxyContent [name='zs_jdy_audit_status']").eq(this.indexEle).val() == "1"){
            if("" != $(obj).val()) {
              ...
              //获得证书id
              var certificateId = this.getCertificateId();
              //获得人员id
...
//          this.$message.success('设置日期成功');
    ...
    //设置页码（当前第几页）
    initPageIndex(){
      var pageObj = $("#xxyContent [name='zs_page_index']");
      for(var i=0;i<pageObj.length;i++){
        pageObj.eq(i).html(i+1) ;
      }
      this.xxyTotalPage = pageObj.length ;
      //设置完总页码后，传输给父组件信息页总页码
      return this.xxyTotalPage ;
    },
    //设置页码（当前第几页）
    getXxyPageTotal(){
      return $("#xxyContent [name='zs_page_index']").length ;
    },
    //设置总页码
    initPageTotal(pageTotal){
      $("#xxyContent [name='zs_page_total']").html(pageTotal);
    },
    //证书添加，信息页保存时候校验
    checkData4CertificateAdd(){
      ...
      if(jsyjRowArr.length > 0 ){
        if($("#xxyContent [name='zs_jsgc_id'][value!='']").length == 0 ){
          return "技术依据不能为空！" ;
        }
        //循环每一行
        for(var i=0;i<jsyjRowArr.length;i++){
          if (jsyjRowArr.eq(i).find("[name='zs_jsgc_id']").eq(0).val() != "") {
            ...
            //寻找英文名称
            if (jsyjRowArr.eq(i).find("[name='zs_jsgc_name_eng']").length == 1) {
              if ("" == $.trim(jsyjRowArr.eq(i).find("[name='zs_jsgc_name_eng']").html())) {
                return "第" + (i + 1) + "行技术依据英文名称不能为空！";
              }
            }
          }
        }
      ...
      if(jlbzRowArr.length > 0 ) {
        if($("#xxyContent [name='zs_jlbz_id'][value!='']").length == 0 ){
          return "计量标准不能为空！" ;
        }
        //循环每一行
        for (var i = 0; i < jlbzRowArr.length; i++) {
          if (jlbzRowArr.eq(i).find("[name='zs_jlbz_id']").eq(0).val() != "") {
            ...
            //寻找英文名称
            if (jlbzRowArr.eq(i).find("[name='zs_jlbz_name_eng']").length == 1) {
              if ("" == $.trim(jlbzRowArr.eq(i).find("[name='zs_jlbz_name_eng']").html())) {
                return "第" + (i + 1) + "行计量标准英文名称不能为空！";
              }
            }
            //有效日期 与 检定日期对比
      ...
      if(bzqRowArr.length > 0 ) {
        if($("#xxyContent [name='zs_jlbzq_id'][value!='']").length == 0 ){
          return "标准器不能为空！" ;
        }
        //循环每一行
        for (var i = 0; i < bzqRowArr.length; i++) {
          if (bzqRowArr.eq(i).find("[name='zs_jlbzq_id']").eq(0).val() != "") {
            ...
            //寻找英文名称
            if (bzqRowArr.eq(i).find("[name='zs_jlbzq_name_eng']").length == 1) {
              if ("" == $.trim(bzqRowArr.eq(i).find("[name='zs_jlbzq_name_eng']").html())) {
                return "第" + (i + 1) + "行标准器英文名称不能为空！";
              }
            }
            //有效日期 与 检定日期对比
    ...
    jdyButtonClick(clickObj){
      ...
      console.log(typeof this.getChangeShr,typeof this.getCertificateId,'111111111111')
      if(typeof this.getChangeShr == "function" && typeof this.getCertificateId == "function"){
    ...
    certificateAudit(auditUserChangeFlag,auditCertificateFlag,auditCertificateInex,auditRyId,certificateId,successTip){
      var returnObj = {};
      var flag = false ;
      ajaxSyncRequest("put", "back/certificate/auditCertificateById", {
    ...
    certificateAuditBatch(djhArr,auditCertificateFlag,successTip){
      var returnObj = {};
      var flag = false ;
      ajaxSyncRequest("put", "back/certificate/auditCertificateBatch/"+djhArr, {auditCertificateFlag:auditCertificateFlag}, res => {
        if(res.code == 200){
          this.$message.success(successTip);
          flag = true ;
        }else {
    ...
    returnCertificatePzr(zsId,returnReason,successTip){
      var returnObj = {};
      var flag = false ;
      ajaxSyncRequest("put", "back/certificate/returnCertificatePzr", {zsId:zsId,returnReason:returnReason}, res => {
        if(res.code == 200){
          this.$message.success(successTip);
          flag = true ;
        }else {
