<template>
  <div :style="{display: 'flex',height:innerHeight+'px'}" ref="temp-page">
      <div class="certificateLeftMenu cerCommonStyle" style="width:250px;height:100%;padding:0px 5px;" v-divDrag="{'drag':_ => calcOtherWidth()}" ref="certLeftMenu">
        <div class="btn">
      ...
      <div class="certificateContent" :style="{'width':calcWidth+'px','padding':'0px 5px','z-index':'1'}">
        <CertificateTemplateDetailMain v-if="isRouterAlive"
                                       :mbId="mbId"
                                       :changeJdyNoCkeck="changeJdyNoCkeck"
        />
        <CertificateTempMlForm v-if="!!edit" :data="edit" :callback="handleEditCallback"/>
        <CertDepartOrFolder  @refreshTree="refreshTree" v-if="!!departEdit" :data="departEdit" :callback="handleEditCallback"></CertDepartOrFolder>
...
<script>
  ...
  import CertificateTemplateDetailMain from './certificateTemplateDetailMain.vue';
  import Clickoutside from '@/common-el/src/utils/clickoutside';
  import CertificateTempMlForm from './certificateTempMlForm';
  ...
  import CertDepartOrFolder from '../../common/certificateCommon/CertDepartOrFolder.vue';
  import $ from 'jquery'
...
/*   import {Container,Aside,Main} from 'element--ui';
  ...
  export default {
    name: "CertificateTempMain",
    components:{
      /** 模块对象 import 进来 后的别名  */
      CertificateListTree,CertificateTemplateDetailMain,CertificateTempMlForm,CertDepartOrFolder,certificateTempSearch
    },
    data(){
      //rentun中的变量仅针对当前组件有效
      return{
...
//        updateNode:null,
    ...
    methods:{
      ...
      leftClick(data ){
        ...
        console.log(this.$refs.certificateListTree.$refs.tree,"this.$refs.certificateListTree.$refs.tree11111111111111111")
        //判断是否模板
        if(data.isLeaf){
...
//        this.mlId = mlId ;
        ...
        ajaxSyncRequest('get', 'back/certificateTemp/'+mlId, null, (res) => {
          if (res.code === 200) {
            ...
            if(this.dataList.length == 1 && mlId == 0){// 如果只存在一个child 则默认展开 并且第一次进入的时候
              setTimeout(val => {
                this.demo = this.$refs.certificateListTree.$refs.tree.$children[0];
      ...
      showRightClickMenu(event,obj,node){
        ...
        if(obj.isParent == '1'){
          ...
          if(this.handType){
            console.log(this.$refs.certificateListTree.getTreeNode(this.currentKey).parent.data.keyMlId,this.$refs.certificateListTree.getTreeNode(this.currentKey).data.keyMlId,'aaa')
            if(this.handType == 4
              &&
              (
                obj.keyMlId == this.$refs.certificateListTree.getTreeNode(this.currentKey).parent.data.keyMlId   //剪切 选择的文件夹非自己的父级
                ||
                obj.keyMlId == this.$refs.certificateListTree.getTreeNode(this.currentKey).data.keyMlId   //剪切 选择的文件夹非自己
              )
        ...
        let RightClickMenu = Vue.extend({
          ...
          render(h){
            return (
              <ul class="right-click-menu cerCommonStyle" v-clickoutside={this.handleClose} style="width:160px">
                {
      ...
      handleDate(rowInfo,_value) {
        ...
        if(!form.mlId){
          ...
          if(form.isParent=='1'){
            ...
            ajaxRequest('post',"back/certificateTemp/addCertificateTempMl",form,res => {
              if(res.code == 200){
          ...
          //模板
            ...
            ajaxRequest('post',"back/certificateTemp/addCertificateTemp",subForm,res => {
              if(res.code == 200){
                var retForm = {};
          ...
          if(form.isParent=='1'){
            ...
            ajaxRequest('put',"back/certificateTemp/updateCertificateTempMl",form,res => {
              if(res.code == 200){
          ...
          //模板
            ...
            ajaxRequest('put',"back/certificateTemp/updateCertificateTemp4Info",subForm,res=>{
              if(res.code == 200){
                this.$message.success('修改成功','update');
...
//        form.type = type ;
        ...
        this.$refs.certificateListTree.appenTreeNode(form);
      ...
      handleDelete (id,keyMlId,isParent) {
        ...
        if (id) {
          if(isParent == 1){
            ...
            //   center:true,
            // }).then(() => {
              ajaxRequest("put", "back/certificateTemp/updateCertificateTempMl4Del", {
                mlId: id,
                status: 1,
                actionType: "目录删除"
              }, function (res) {
                if (res.code == 200) {
                  ...
                  _this.$refs.certificateListTree.delTreeNode(keyMlId);
            ...
            // })
              ...
              ajaxRequest("put", "back/certificateTemp/updateCertificateTemp4Del", {
                mbId: id,
                status: 1,
                actionType: "模板删除"
              }, function (res) {
        ...
        if (id) {
          ...
          if(this.handType == 3){
            if(_this.dealTreeNodeType == 1){
              ...
              var treeTempOld = _this.$refs.certificateListTree.getTreeNode(_this.currentKey);
              console.log(treeTempOld.parent.label,'treeTempOld')
              var treeTempNew = _this.$refs.certificateListTree.getTreeNode(keyMlId);
              console.log(treeTempNew.label,'treeTempNew')
              ajaxRequest("put", "back/certificateTemp/copyCertificateTempByIds",{mbId:_this.copyId,mlId:id,actionType:"模板复制",actionContent:(treeTempOld.parent.label + "->" + treeTempNew.label)}, function(res) {
                if (res.code == 200) {
//                  var newDate = res.daoResult ;

                  ajaxSyncRequest('get', 'back/certificateTemp/'+id, null, (res) => {
                    if (res.code === 200) {
...
//                  _this.$refs.certificateListTree.appenTreeNode(newDate);
            ...
            if(_this.dealTreeNodeType == 1){
              ...
              ajaxRequest("put", "back/certificateTemp/moveTempleteDirectory",{mlId:_this.copyId,mlIdParent:id,actionType:"目录移动",actionContent:(treeTempOld.parent.label + "->" + treeTempNew.label)}, function(res) {
                if (res.code == 200) {
                  //获得原来的节点
//                  var treeTempOld = _this.$refs.certificateListTree.getTreeNode(_this.currentKey);

                  _this.$refs.certificateListTree.delTreeNode(_this.currentKey);
...
//                  _this.$refs.certificateListTree.appenTreeNode(treeTempOld.data);
//                  console.log(treeTempOld.data,'333333')
////                  _this.$refs.certificateListTree.reFreashParentTreeNode(treeTempOld.data);
//
//                  _this.copyId = null ;
//                  _this.handType=null ;
//                  _this.isParent=null ;
...
//                  _this.$message.success('移动成功')
              ...
              console.log(treeTempNew.label,'treeTempNew')
              //更新后台
              ajaxRequest("put", "back/certificateTemp/updateCertificateTemp4Move",{mbId:_this.copyId,mlId:id,actionType:"模板移动",actionContent:(treeTempOld.parent.label + "->" + treeTempNew.label)}, function(res) {
                if (res.code == 200) {
                  //获得原来的节点
//                  var treeTempOld = _this.$refs.certificateListTree.getTreeNode(_this.currentKey);
...
//        this.departEdit=null;
        this.$refs.certificateListTree.updateNode(pKey,data);
