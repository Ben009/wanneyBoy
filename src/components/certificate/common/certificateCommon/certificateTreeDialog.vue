...
//          this.currentKey = this.zmbId;
      ...
      getDataList() {
        let _this = this;
        ajaxRequest('get', 'back/certificateMainTemp/list', {queryType:this.queryType,zmbId:this.xxyZmbId,isFirst:this.isFirst}, (res) => {
          if (res.code === 200) {
//            let dataTreeList = this.formatArrayToTree(res.rows, [], 0, []);
            if(this.queryType == 1){
              res.rows.push({"zmbType":0,"zmbName":"信息页","zmbId":1});
            }else if(this.queryType == 2){
              res.rows.push({"zmbType":0,"zmbName":"结果页","zmbId":2},);
            }
