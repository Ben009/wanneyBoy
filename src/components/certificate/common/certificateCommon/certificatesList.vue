  ...
  <div class="certificatesListStyle">
    <div class="topButtom">
      <div class="certificateNumber">
        ...
        <el-tooltip content="应出证书数量/已出证书数量" placement="top">
    ...
    <div class="certificateNeedAduit">
      <ul style="/*overflow: auto;*/font-size:14px;width:100%;">
        <!-- 列表 -->
        <li v-for="val in certificateList"
            style="cursor:pointer;width:100%">
          <div :class="{cursor:true,'current-row':certificateId && certificateId == val.zsId }" class="listItem">
            <span v-if="val.returnFlag == 1 " style="color: red;" :style="{marginRight : typeof handleDelete == 'function' ? '10px' : '0px'}" @click="quoInfoClick(val.zsId)">{{zsbhFormat(val.zsbh)}}</span>
            <span v-else="" :style="{marginRight : typeof handleDelete == 'function' ? '10px' : '0px'}" @click="quoInfoClick(val.zsId)">{{zsbhFormat(val.zsbh)}}</span>

            <el-button v-if="typeof handleDelete == 'function'" v-mindPopover="{message:'是否删除?',success:handleDelete.bind(this,val.zsId)}" type="text" style="float: right;padding:0;font-size:14px;">
...
<script>
  ...
  import CertificateEditNumber from './certificateEditNumber';
  ...
  export default {
    ...
    methods: {
      quoInfoClick(zsId){
        this.$emit('leftCilck', zsId );
      },
//      handleDelete(zsId){
//        this.handleDelete(zsId);
////        this.$emit('handleDelete', zsId );
//      }
      //修改证书数量(弹窗显示)
      handleEditNumber(){
