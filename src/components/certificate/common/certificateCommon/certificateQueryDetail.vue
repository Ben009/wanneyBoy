<template>
  <div>
    <div class="topButton" ref="topButton">
      <el-form :inline="true">
        <el-form-item v-show="form.ysjlId != null " class="cerCommonStyle">
          <span  >
            <span style="color: red">（原始记录生成）</span>
          </span>
        </el-form-item>
        <!-- 是否建标 -->
        <el-form-item class="cerCommonStyle" v-show="canChangeCertificateisAuthorise">
            <span v-show="form.isAuthorise == 1" ><span style="color: red">SC0</span></span>
            <span v-show="form.isAuthorise == 0" >SC1</span>
        </el-form-item>
        <!-- 是否CNAS -->
        <el-form-item class="cerCommonStyle">
          是否CNAS：
            <span v-show="form.cnasFlag == 1" label="否">否</span>
            <span v-show="form.cnasFlag == 0" label="是">是</span>
        </el-form-item>
        <!-- 是否强检 -->
        <el-form-item prop="qjFlag" class="cerCommonStyle" >
          是否强检：
            <span v-show="form.qjFlag == 0" label="否">否</span>
            <span v-show="form.qjFlag == 1" label="是">是</span>
        </el-form-item>
        <!-- 器具用途 -->
        <el-form-item class="cerCommonStyle" v-show="form.qjFlag == 1">
          器具用途：
            <span v-for="applianceUse in applianceUses"
                       v-show="form.qjyt == applianceUse.code "  >
              {{applianceUse.remark}}
            </span>
        </el-form-item>
        <!-- 收费情况说明 -->
...
<!--         <el-col :span="24">
    ...
    <div>
      <iframe id="iframeDialog" :srcdoc="demo" :style="{height:iframeHeight+'px'}"></iframe>
    </div>
  </div>
</template>
...
<script>
  ...
  import $ from 'jquery';
  import CertificateXxy from '../../certificate/certificateTemp/certificateXxy.vue';
  import CertificateJgy from '../../certificate/certificateTemp/certificateJgy.vue';
    ...
    methods: {
        calcIframeHeight(){
          this.$nextTick(val => {
            ...
            let dialogBodyHeight = $(dialog).height() - $(dialog.querySelector('.el-dialog__header')).outerHeight(true);
            console.log($(this.$refs.topButton),$(this.$refs.topButton).outerHeight(true) ,this.$refs.topButton.clientHeight)
            this.iframeHeight = dialogBodyHeight - this.$refs.topButton.clientHeight - 10
          })
        },
//      handleSubmit() {
//
...
//              this.certificateTempId = this.form.mbId ;
              ...
              //检定周期
              // this.$refs.certificateXxy.certificateJdzq = this.form.jdzq
              this.demo = this.form.xxyContent + this.form.jgyContent;
              // console.log($('#iframeDialog')[0].contentWindow,'111111111111111')
              //初始化Excel参数
//              this.form["caseId"] = this.registrationNo
//              this.initExcelInfo(this.form);
