<template>
  <div  style="height:100%;display: flex" class="firstAuditCss" ref="firstAudit-page">
      ...
      <div class="certificateContent" :style="{'width':calcWidth+'px','padding':'0px 5px','z-index':'1'}">
        <OriginalLogTemplateTwoAuditDetail v-if="isRouterAlive" :mlId="mlId" :mbId="mbId"
...
<script>
  ...
  import OriginalLogTemplateTwoAuditDetail from './originalLogTemplateTwoAuditDetail.vue';
...
/*   import {Container,Aside,Main} from 'element--ui';
  ...
  export default {
    name: "OriginalLogTempMainTwoAudit",
    components:{
      /** 模块对象 import 进来 后的别名  */
      CertificateListTree,OriginalLogTemplateTwoAuditDetail,
    },
    data(){
      //rentun中的变量仅针对当前组件有效
      return{
     ...
     getUnauditedList(mlId){
       if(mlId){
         this.checkUnauditedListInterim = [] ;
          ajaxRequest("get", "back/originalLogTemp/getOriginalLogTempListByMlId",{mlId:mlId,auditFlag:2}, (res) => {
             if(res.code === 200){
     ...
     getAuditedList(mlId){
       if(mlId){
         this.checkAuditedListInterim = [] ;
          ajaxRequest("get", "back/originalLogTemp/getOriginalLogTempListByMlId",{mlId:mlId,auditFlag:3}, (res) => {
             if(res.code === 200){
