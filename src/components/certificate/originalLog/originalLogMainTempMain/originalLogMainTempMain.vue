<template>
    <div ref="mainTemp-page" style="height:100%;display: flex">
      <div class="certificateLeftMenu" style="width:250px;padding:0px 5px;" v-divDrag="{'drag':_ => calcOtherWidth()}" ref="certLeftMenu">
        <CertificateListTree  ref="originalLogListTree"  :dataList="dataList" :keyId="keyId" :treePid="treePid" :propsKey="propsKey" :currentKey="currentKey"
      ...
      <div class="certificateContent" :style="{'width':calcWidth+'px','padding':'0px 5px','z-index':'1'}">
        <OriginalLogMainTemplateDetail v-if="!!isRouterAlive" :zmbId="zmbId" />
...
//          this.currentKey = this.zmbId;
      ...
      getDataList() {
        ajaxRequest('get', 'back/originalLogMainTemp/list', {
          status:0,
        }, (res) => {
      ...
      showRightClickMenu(event,obj,node){
        ...
        let RightClickMenu = Vue.extend({
          ...
          template:`<ul class="right-click-menu" v-clickoutside="handleClose">
                      <li v-if="${obj.zmbType} == 0" v-mindPopover="{message:'<div>新增原始记录模板</div><div><el-input v-model=flag3 v-focusSelectFull></el-input></div>',success:handleRightClickOperation.bind(this,0),info:'',placement:'bottom',isOpen:handleOpen.bind(this)}">增加模板</li>
                      <li v-if="${temp} == true" @click="handleRightClickOperation(4)">粘贴</li>
                      <li v-if="${obj.zmbType} == 1 || ${obj.zmbType} == 2" v-mindPopover="{message:'<div>重命名原始记录模板名称为</div><div><el-input v-model=flag3 v-focusSelectFull></el-input></div>',success:handleRightClickOperation.bind(this,1),info:'${obj.zmbName}',isOpen:handleOpen.bind(this)}">重命名</li>
      ...
      handleEdit(rowInfo,op,_value){
        ...
        if(_value.flag3 == ''){
          this.$message.error('原始记录模板名称不能为空')
          return true
        }
        let _this = this;
        if (op == 0) {
          ajaxRequest("post", "back/originalLogMainTemp/", {zmbType:rowInfo.zmbId,zmbName:_value.flag3}, function(res) {
            if (res.code == 200) {
                _this.$message.success('添加成功');
                _this.$refs.originalLogListTree.updateNode(res.daoResult.zmbType,res.rows);
            } else {
          ...
          //把时间字段删掉
          ajaxRequest("post", "back/originalLogMainTemp/renameZmbName",{_method:'post',zmbId:rowInfo.zmbId,zmbName:_value.flag3,backupType:1,zmbType:rowInfo.zmbType}, function(res) {
            if (res.code == 200) {
      ...
      handleEditCallback(key,children){
        this.edit=null;
        this.$refs.originalLogListTree.updateNode(key,children);
      },
      handleOperation (obj,node, op,_this){
        if(op == 3){
          //删除模板
      ...
      handleDelete (data) {
        ...
        if (data.zmbId) {
          ...
          //             center:true,
          //           }).then(() => {
                      ajaxRequest("post", "back/originalLogMainTemp/deleteZmb",{_method:'post',zmbId:data.zmbId,status:1,backupType:0,zmbType:data.zmbType}, function(res) {
                         if (res.code == 200) {
      ...
      handlePaste(copyZmbId) {
        ...
        if (copyZmbId) {
          //先根据copy模板的ID进行查询
//           ajaxRequest('get', 'back/originalLogMainTemp/'+copyZmbId,null, (res) => {
//            if (res.code === 200) {
//              if(res.daoResult){
//                  //查询出来进行添加操作
//                  //把时间字段删掉
...
//                  delete res.daoResult.dataDate1
                    ajaxRequest("post", "back/originalLogMainTemp/copyOriginalLogById", {copyZmbId:copyZmbId}, function(res) {
                      if (res.code == 200) {
                        _this.handleEditCallback(res.daoResult.zmbType,res.rows);
