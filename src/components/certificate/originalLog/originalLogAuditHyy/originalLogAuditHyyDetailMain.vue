...
<script>
  ...
  export default {
    name: "OriginalLogAuditHyyDetailMain",
    components:{
      /** 模块对象 import 进来 后的别名  */
...
//        this.$refs.certificateTemplateDetail.changeAuditFlag(type,step,actionType);
      ...
      certificateSH(){
        ...
        for(var i=0;i<this.auditStatusTemp.length;i++){
          if(this.auditStatusTemp[i].auditRyId == window.localStorage.userId && this.auditStatusTemp[i].auditRyStatus != 1 && flag){
            flag = this.$refs.originalLogAddDetail.$refs.originalLogXxy.checkSignnaturePasswordBack(true,"0","1",i,this.auditStatusTemp[i].auditRyId ,this.originalLogId,"")
          }
        }
        if(flag){
      ...
      certificateSHBatch(){
        ...
        var auditObj = this.$refs.originalLogAddDetail.$refs.originalLogXxy.certificateAuditBatch(this.checkAuditDjhList,'1',"审核成功");
        if(auditObj != false){
      ...
      getAuditStatus(auditStatus){
        this.auditStatusTemp = auditStatus.hyy ;
        for(var i=0;i<this.auditStatusTemp.length;i++){
