      ...
      <tr v-for="head in headList">
        <td v-for="(item,index) in head" :required="item.name === 'applianceUse' ? applianceRequired : item.required" :name="item.name" :colspan="item.colspan" :rowspan="item.rowspan">
          ...
          <div v-if="item.dom === 'inputNumber'">
            ...
            <el-input-number  v-else-if="item.name=='afterDiscountPrice'" v-model.number="form[item.name]"
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      quotationDecimals: {required: true, type: Number},
      wtdInfo:{type:Object}
    },
    data() {
      return {
    ...
    methods: {
      ...
      handleSetNumberChange(value) {
        ...
        this.$nextTick(() => {
          let afterDiscountPrice = this.setDecimalsPoint(this.form.afterDiscountPrice, this.quotationDecimals)// 折后单价
          let setNumber = new Big(value)// 套数
          let pieceNumber = new Big(this.form.pieceNumber)// 件数
          let total = new Big(afterDiscountPrice.toString()).mul(setNumber).mul(pieceNumber)// 合计
          Vue.set(this.form, 'afterDiscountTotal', total.toString())
          this.$emit('setDecimals')
        });
      },
      ...
      handlePieceNumberChange(name, type, value) {
        ...
        this.$nextTick(() => {
          let afterDiscountPrice = this.setDecimalsPoint(this.form.afterDiscountPrice, this.quotationDecimals)// 折后单价
          let setNumber = new Big(this.form.setNumber);// 套数
          let pieceNumber = new Big(this.form.pieceNumber);// 件数
      ...
      /* 修改折后价格，重新计算合计*/
      handleSetAfterPrice(v) {
        this.$emit('setDecimalsByAfter')
      },
      /* 检验证书数量是否大于套数 */
      ...
      setValue(row) {
        if (row) {
          ...
          row.afterDiscountPrice = this.setDecimalsPoint(row.unitPrice, this.quotationDecimals)// 折后单价
          /* 重新拉取岗位 */
