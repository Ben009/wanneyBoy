<!--
 ...
 -->
<template>
  <div style="position:relative;" class="conductUser">
            ...
            <WorkMould v-if="isSetting" :rowsCondition='paramJson' :callback="QJPersonalSetting"
             ...
             <div style="float:right;margin-top: 5px">
              <global-tootips>
                ...
                <p>4、	<span style="color: red">检测期限红色</span>：已超期；证书状态（A/B/C）A表示应出证书份数、B表未已出证书份数，C表示已批准证书份数；库房状态（D/E）D表示证书库房，E表示器具库房。</p>
              </global-tootips>
            </div>
      ...
      <el-table :max-height="QJTableMaxHeight"
        ...
        >
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.isUrgent>1" style="color: red;">是</span>
          </template>
        </el-table-column>
...
<script>
  ...
  import paramJson from './paramTasks'
  ...
  export default {
    ...
    methods: {
      ...
      getTableList() {
        ajaxRequest('get', 'back/secondAllot/getConductUser', {
          fuzzySearch: 1,
          page: this.page,
          rows: this.rows,
