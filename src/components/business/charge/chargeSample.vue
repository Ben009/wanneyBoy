  ...
  <mind-dialog
    ...
    <div v-if="chargedList.length>0">
      <div class="title-item">
        ...
        <el-button type="danger" size="mini" @click="cancelSampleCharge">撤销收费</el-button>
      </div>
      <div style="padding-bottom: 10px;padding-top: 10px">
        <el-checkbox-group v-model="chargedListChecked" @change="handleCheckedChargedChange">
          <el-checkbox v-for="charged in chargedList" :key="charged.sampleBh" :label="charged.sampleBh" >{{charged.sampleBh}}<span style="color: red">({{charged.jdfActual}})</span></el-checkbox>
        </el-checkbox-group>
      </div>
    </div>
    <div v-if="unChargeList.length>0">
      <div class="title-item"><span>未收费区&nbsp;&nbsp;&nbsp;<el-checkbox v-model="unChargCheckAll" @change="handleUnChargCheckAllChange">全选</el-checkbox></span></div>
      <div style="padding-bottom: 15px;padding-top: 10px">
        <el-checkbox-group v-model="unChargeListChecked" @change="handleCheckedUnChargChange">
          <el-checkbox v-for="(unCharge,index) in unChargeList" :key="unCharge.sampleBh" :label="unCharge.sampleBh">{{unCharge.sampleBh}}</el-checkbox>
        </el-checkbox-group>
      </div>
    </div>
    ...
    <div style="margin-top: 10px">
      ...
      <el-Row style="font-weight: bold;font-size: 14px;padding-top: 10px">
        <el-col :span="3" style="text-align: right">
          已录入合计：
        </el-col>
        <el-col :span="4">
    ...
    <span slot="footer" class="dialog-footer">
        <el-button size="mini" class="el-button el-button--primary" type="primary" @click="handleSubmit">保 存</el-button>
        <el-button size="mini" class="el-button " @click="handleCancel">取 消</el-button>
    </span>
  </mind-dialog>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        chargedCheckAll: false,//已收费区全选
        unChargCheckAll: true,//未收费区全选
    ...
    methods: {
      ...
      ajaxSampleList(){
        let _this = this;
        ajaxSyncRequest('get','back/charge/queryRegistrationNoSample',{registrationNo:this.registrationNo},function(res){
          ...
          _this.handleUnChargCheckAllChange();
      ...
      handleSubmit() {
        let info={};
        let _this=this;
        if(_this.chargedListChecked.length>0 && _this.unChargeListChecked.length>0){
          _this.$message.error('已收费区域和未收费区域不能同时选中');
          return;
        }
        if(_this.chargedListChecked.length==0 && _this.unChargeListChecked.length==0){
          _this.$message.error("请选择需要收费的器具");
          return;
        }
        ...
        ajaxRequest('post','back/charge/doAddOrUpdateSampleCharge',{json:JSON.stringify(info)},function(res){
          if(res.code == 200){
            if(res.msg.indexOf("已录费")!=-1){
              _this.$message.error('登记号已录入费用，请重新刷新页面！');
              return;
            }else{
              _this.$message.success('器具收费成功');
              _this.ajaxSampleList();
              if(_this.unChargeList.length===0){
                _this.handleCancel();
              }
            }
      ...
      cancelSampleCharge() {
        ...
        let _this=this;
        if(_this.chargedListChecked.length==0){
          _this.$message.error('请选择需要撤销收费的器具');
        }else{
          info.fjd=0;
          info.fjf=0;
          info.fjfActual=0;
      ...
      handleCancel(){
        let _this=this;
        ajaxRequest('get','back/charge/getRegistrationNoTotalFee',{registrationNo:this.registrationNo},function(res){
          if(res.code == 200){
            let data=res.daoResult;
            if(_this.chargedList.length>0){
              data.feeType='1';//费用录入方式：1样品，0非样品
            }else{
              data.feeType='0';
            }
            if(res.map.msgFlag=="1"){
              _this.$confirm('存在未录入费用的器具，无法提交费用信息，确认取消?', '提示', {
      ...
      //未收费区域全选按钮事件
      handleUnChargCheckAllChange() {
        let temp= [];
        this.unChargeList.forEach(val => {
          temp.push(val.sampleBh)
        })
        if(!this.unChargCheckAll){
          temp=[];
        }
        this.unChargeListChecked = temp.length>0 ? temp: [];
      },
      //已收费区域全选按钮事件
      handleChargedCheckAllChange(val) {
        let temp= [];
        this.chargedList.forEach(val => {
          temp.push(val.sampleBh)
        })
        this.chargedListChecked = val ? temp : [];
      },
      //未收费区域勾选事件
      ...
      //已收费区域勾选事件
      handleCheckedUnChargChange(value) {
        let checkedCount = value.length;
        this.unChargCheckAll = checkedCount === this.unChargeList.length;
      },
      //计算加急费
      toSynchronJjf(){
