    ...
    <el-form ref="form" class="sampleForm">
      <div>
        <el-Row :gutter="20">
          ...
          <el-col :span="6">
            <el-form-item label="批准机关" class="lbCenter"></el-form-item>
          </el-col>
        </el-Row>
        <el-Row :gutter="20">
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item>
              <el-select v-model="approvalAuth" placeholder="请选择">
                <el-option
                  v-for="item in approvalAuthOptions"
                  :key="item.code"
                  :label="item.remark"
                  :value="item.code">
                </el-option>
      ...
      <div>
        <el-table :max-height="QJTableMaxHeight"
                  ...
                  style="width: 100%"
                  :fit="true">
          <el-table-column prop="standardName" label="计量标准名称"></el-table-column>
          <el-table-column prop="departName" label="维护部门"></el-table-column>
          <el-table-column prop="standardCerNo" label="计量标准证书号"></el-table-column>
          <el-table-column prop="standardValidDate" label="计量标准有效期" :formatter="dealDate"></el-table-column>
          <el-table-column prop="socCerNo" label="社会公用证书号" v-if="headObj.socCerNo"></el-table-column>
          <el-table-column prop="text" label="批准机关" v-if="headObj.text"></el-table-column>
          <el-table-column prop="socValidDate" label="社会公用证书有效期" v-if="headObj.socValidDate" :formatter="dealDate"></el-table-column>
          <el-table-column prop="classAccuracy" label="准确度等级" v-if="headObj.classAccuracy"></el-table-column>
          <el-table-column prop="measuringRange" label="测量范围" v-if="headObj.measuringRange"></el-table-column>
          <el-table-column prop="userName" label="负责人" v-if="headObj.userName"></el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  import paramJson from "./standardParam"; //同目录下json数据
  import charts from "./charts"; //图表
  export default {
    name:"standardWarning",//计量标准标准预警
    components: {
      WorkMould,
      TableColumnHeader,
      Retrieve,
    ...
    data() {
      return {
        ...
        approvalAuthOptions:[],//批准机关option
        departId:"",//检测部门id
        approvalAuth:"",//批准机关id
    ...
    methods:{
      getTableList(){
        ajaxRequest('get','back/statistics/standardWarning',{
          departId:this.departId,//检测部门id
          approvalAuth:this.approvalAuth,//批准机关id
        },(res)=>{
          if(res.code===200){
      ...
      setOptions(){
        ...
        ajaxProxyRequest('get','/njmind/findParams/approvalAuth',{codeType:'NUMBER'},(res) => {
          this.approvalAuthOptions=res.approvalAuth.list;
        })
      },
      exportList(){//导出
