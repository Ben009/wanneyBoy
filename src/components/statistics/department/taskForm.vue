    ...
    <mind-dialog
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        <el-table  :max-height="QJTableMaxHeight"
          ...
          <el-table-column label="是否配号">
            <template slot-scope="scope">
              <span v-if="scope.row.isHaveNumber==0">否</span>
              <span v-if="scope.row.isHaveNumber==1">是</span>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    methods:{
      getTableList(){
        ...
        this.data.total=0;
        /*获取table数据，维护data数据*/
        ajaxRequest('get','back/statistics/xianchangCountDetail',this.data,(res)=>{
          if(res.code===200){
